...
type Directories = operations["list_directories"]["responses"]["200"]["content"]["application/json"]

export const load: PageLoad = async ({ fetch }) => {
  let client = createClient({ fetch })

  let directories = await loadCreate(client)
...
async function loadCreate(client: ReturnType<typeof createClient>): Promise<Directories> {
  let res = await client.POST("/api/v1/filesystem/directories/list", { body: { path: "" } }).catch(() => {
    loadCreateError(504)
  })
