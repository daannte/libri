...
async function loadLibraryMedia(client: ReturnType<typeof createClient>, id: number): Promise<LibraryMediaResponse> {
  let res = await client.GET("/api/v1/libraries/{id}/media", { params: { path: { id } } }).catch(() => {
